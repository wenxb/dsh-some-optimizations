...
//   1. 模型钉死 (modelPin) — pin each picked session to its selected model (session-scoped)
...
var CLICK_URL = '/dsh-some-optimizations/click';

// ===== click observer（会话级钉子的数据源）=====
// 包一层 window.fetch，旁观所有请求体：发现 `session.selectModel` rpc
// （含 sessionId/provider/model）就 fire-and-forget 上报宿主 /click 路由。
// 纯观察——原调用原样透传，绝不改写；HMR 重载靠窗口标志防重复包裹。
function installClickObserver() {
  if (typeof window === 'undefined' || typeof window.fetch !== 'function') return;
  if (window.__dsoClickObserver) return;
  window.__dsoClickObserver = true;
  var nativeFetch = window.fetch;
  window.fetch = function (input, init) {
    try {
      var body = init && typeof init.body === 'string' ? init.body : '';
      if (body.indexOf('session.selectModel') !== -1) {
        reportSelection(selectionFromBody(body));
      }
    } catch (_) { /* 观察失败不影响请求 */ }
...
function selectionFromBody(body) {
  var parsed;
  try { parsed = JSON.parse(body); } catch (_) { return undefined; }
  // rpc 载荷可能在顶层或 payload 字段里，两处都试。
  var candidates = [parsed && parsed.payload, parsed];
  for (var i = 0; i < candidates.length; i++) {
    var p = candidates[i];
    if (p && typeof p.sessionId === 'string' && p.sessionId &&
        typeof p.provider === 'string' && p.provider &&
        typeof p.model === 'string' && p.model) {
      var out = { sessionId: p.sessionId, provider: p.provider, model: p.model };
      if (typeof p.reasoningEffort === 'string') out.reasoningEffort = p.reasoningEffort;
      return out;
    }
  }
  return undefined;
...
function reportSelection(selection) {
  ...
  }).catch(function () { /* 宿主路由暂不可用时静默；下次点选再试 */ });
}
...
var SPECS = [
  {
    field: 'modelPin',
    title: '模型钉死（model-pin，按会话）',
    desc: '在某个会话里点选模型后，该会话之后的所有请求都强制走所选模型，无视任何中间层改写；其他会话不受影响。',
  },
  {
